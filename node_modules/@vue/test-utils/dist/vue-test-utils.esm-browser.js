...
/**
 * @vue/test-utils v2.0.0-rc.18
 * (c) 2021 Lachlan Miller
 * Released under the MIT License
 */

import * as Vue from 'vue';
import { transformVNodeArgs, Transition, defineComponent, TransitionGroup, Teleport, h, nextTick, setDevtoolsHook, reactive, createApp } from 'vue';
...
var createStub = function (_a) {
    var name = _a.name, propsDeclaration = _a.propsDeclaration, renderStubDefaultSlot = _a.renderStubDefaultSlot;
    ...
    var render = function (ctx) {
        ...
        var propsWithoutSymbols = stringifySymbols(ctx.$props);
...
var createTeleportStub = function (_a) {
    var name = _a.name;
    var render = function (ctx) {
        return h(name, {}, ctx.$slots);
    };
...
var getComponentName = function (instance, type) {
    if (isObjectComponent(type)) {
        var defaultName = Object.keys((instance === null || instance === void 0 ? void 0 : instance.setupState) || {}).find(function (key) { return instance.setupState[key] === type; });
...
function stubComponents(stubs, shallow, renderStubDefaultSlot) {
    ...
    transformVNodeArgs(function (args, instance) {
        ...
        if (isComponent$1(type) || isFunctionalComponent(type)) {
            ...
            if (stub === true || shallow) {
                ...
                var propsDeclaration_1 = unwrapLegacyVueExtendComponent(type).props || {};
                var newStub = createStubOnce(type, function () {
                    return createStub({
                        name: stubName_1,
                        propsDeclaration: propsDeclaration_1,
                        renderStubDefaultSlot: renderStubDefaultSlot
                    });
                });
...
var BaseWrapper = /** @class */ (function () {
    ...
    BaseWrapper.prototype.find = function (selector) {
        if (typeof selector === 'object' && 'ref' in selector) {
            ...
            if (result_1 instanceof Node) {
                return createDOMWrapper(result_1);
            }
            else {
                return createWrapperError('DOMWrapper');
            }
        }
        var elementRootNodes = this.getRootNodes().filter(function (node) { return node instanceof Element; });
        if (elementRootNodes.length === 0) {
            return createWrapperError('DOMWrapper');
        }
        var matchingRootNode = elementRootNodes.find(function (node) {
            return node.matches(selector);
        });
        if (matchingRootNode) {
            return createDOMWrapper(matchingRootNode);
        }
        var result = elementRootNodes
            .map(function (node) { return node.querySelector(selector); })
    ...
    BaseWrapper.prototype.findAll = function (selector) {
        if (!isElement(this.element)) {
            return [];
        }
        var result = this.element.matches(selector)
            ? [createDOMWrapper(this.element)]
            : [];
        return __spreadArray(__spreadArray([], result, true), Array.from(this.element.querySelectorAll(selector)).map(function (x) {
            return createDOMWrapper(x);
        }), true);
    };
    BaseWrapper.prototype.findComponent = function (selector) {
        ...
        if (typeof selector === 'object' && 'ref' in selector) {
            var result_2 = currentComponent.refs[selector.ref];
            if (result_2 && !(result_2 instanceof HTMLElement)) {
                return createVueWrapper(null, result_2);
            }
            else {
                return createWrapperError('VueWrapper');
            }
...
function getRootNodes(vnode) {
    if (vnode.shapeFlag & (1 /* ELEMENT */ | 128 /* SUSPENSE */)) {
        return [vnode.el];
    }
        ...
        var children = vnode.children.flat(Infinity);
        return children
    ...
    return {
        emit: function (eventType) {
            ...
            if (eventType !== "component:emit" /* COMPONENT_EMIT */)
                return;
            payload[0]; var componentVM = payload[1], event = payload[2], eventArgs = payload[3];
...
export { DOMWrapper, RouterLinkStub, VueWrapper, config, createWrapperError, disableAutoUnmount, enableAutoUnmount, flushPromises, mount, shallowMount };
