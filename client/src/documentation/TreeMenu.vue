...
import { ref } from 'vue'
import { formatId } from './documentation_tree2'

defineProps(['label', 'nodes', 'depth'])

const itemDicts = ref(parse(doctext))
...
<template>
  <div class="tree-menu">
    <b-card
      class="border-end-0 border-top-0 border-bottom-0 border-5 bg-secondary"
    >
      <b-card-title v-b-toggle="formatId(label)" style="cursor: pointer"
        >{{ label }}
        <b-button v-b-toggle="formatId(label)" variant="secondary">
      ...
      <b-collapse :id="formatId(label)" class="collapse">
