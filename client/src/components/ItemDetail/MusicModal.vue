        ...
        <b-container class="p-0">
            ...
            <b-row v-if="lastFmTrack" class="mt-2 mx-1">
              ...
              <b-col class="p-0">
                <b-card class="bg-secondary border-0">
                  <b>LastFM tags:</b>
                  <b-badge variant="dark" v-for="item in lastFmTrack.toptags.tag" :key="item" class="d-inline ms-2 p-2">
                    <a :href="item.url" class="text-white" style="font-size: 14px;">{{ item.name }}</a>
                  </b-badge>
                </b-card>
              </b-col>
            </b-row>
          ...
          <b-row class="py-2 mx-1">
            ...
            <b-collapse id="collapse-1">
              <h6 class="mt-1">
                debug | id: <i>{{ track.id }}</i><br>
                preview url: <a :href='track.preview_url'>{{ track.preview_url }}</a>
              </h6>
              <p class="m-0" v-for="[key, value] of Object.entries(track)" :key="key">
                <b>{{ key }}</b
                >: {{ value }}
              </p>
            </b-collapse>
